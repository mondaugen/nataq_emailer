...
#echo "${FILENAME}"
#echo "${out_file}"
ffmpeg -i "${FILENAME}" -c:a libmp3lame -b:a 320k "${tmp_out_file}"

ffmpeg -i "${tmp_out_file}" -i "${ALBUM_PICTURE}" -codec copy -map 0:0 -map 1:0 -id3v2_version 3 \
